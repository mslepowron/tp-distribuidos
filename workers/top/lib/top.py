...
class Top:
    ...
    def _send_rows(self, header, rows, routing_keys, query=None):
        if not rows:
            return
        try:
            ...
            out_header = Header({
                ...
                "query_id":query if query is not None else header.fields["query_id"],
                "stage": "Top",
    ...
    def _forward_eof(self, header, stage, routing_keys=None, query=None):
        try:
            out_header = Header({
                "message_type": header.fields["message_type"],
                "query_id": query if query is not None else header.fields["query_id"],
    ...
    def callback(self, ch, method, properties, body):
        try:
            ...
            if header.fields.get("message_type") == "EOF":
                ...
                self._send_rows(header, result_rows, self.output_rk, self.output_rk)
                self._forward_eof(header, "TopSellingItems", routing_keys=self.output_rk, query=self.output_rk)
    ...
    def callback(self, ch, method, properties, body):
        try:
            ...
            if header.fields.get("message_type") == "EOF":
                ...
                logger.info(f"RESULT ROWS: {result_rows}")
                self._send_rows(header, result_rows, self.output_rk, self.output_rk)
                self._forward_eof(header, "TopRevenueGeneratingItems", routing_keys=self.output_rk, query=self.output_rk)
