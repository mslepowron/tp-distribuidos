...
class Reduce:
    ...
    def _send_rows(self, header, source, rows, routing_keys, query=None):
        if not rows:
            return
        try:
...
class TpvReducer(Reduce):
    ...
    def callback(self, ch, method, properties, body):
        try:
            ...
            if header.fields.get("message_type") == "EOF":
                ...
                if self.eofs_received >= self.eofs_expected:
                    self._emit_result(header)
                    self._forward_eof(header, "TpvReducer", self.output_rk, self.output_rk)
                return
    ...
    def _emit_result(self, header):
        ...
        self._send_rows(header, "tpv_reduce", result_rows, routing_keys=self.output_rk, query=self.output_rk)