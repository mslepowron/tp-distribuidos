...
class Sender:
    ...
    def wait_for_reports(self):
        OUTPUT_DIR = Path("/salida_cliente")
        ...
        while True:
            ...
            if not data:
                logger.info("Conexión cerrada por el Gateway (no hay más reportes)")
                break
            ...
            if text.startswith("FILENAME:"):
                ...
                parts = header.split(";")
                fname = parts[0].replace("FILENAME:", "").strip()
                batch = int(parts[1].replace("BATCH:", "").strip())

                if fname not in buffers:
                    buffers[fname] = []
                buffers[fname].append(content)

                logger.info(f"Chunk recibido de {fname} (batch {batch})")

            elif text.startswith("EOF:"):
                fname = text.replace("EOF:", "").strip()
                if fname in buffers:
                    out_path = OUTPUT_DIR / fname
                    with open(out_path, "w", encoding="utf-8") as f:
                        f.write("".join(buffers[fname]))
                    logger.info(f"Reporte completo guardado en {out_path}")
                    del buffers[fname]
                else:
                    logger.warning(f"EOF recibido para {fname} sin datos previos")
            else:
                logger.warning(f"Formato inesperado: {text[:50]}")
